...
const createProduct = asyncHandler(async (req, res) => {
    const { name, sku, category, subcategory, subSubcategory, brand, price, pricePerCase, packSize, unit, description, stockQuantity, minStock, } = req.body;
...
const updateProduct = asyncHandler(async (req, res) => {
    const { name, sku, category, brand, price, pricePerCase, packSize, unit, description, stockQuantity, } = req.body;
    const product = await Product.findById(req.params.id);
    if (!product) {
        res.status(404);
        throw new Error("Product not found");
...
const getAvailableCategories = asyncHandler(async (req, res) => {
    try {
        ...
        const categoriesWithSubcategories = categories.map((category) => {
            ...
            const subcategoriesWithSubSubcategories = categorySubcategories.map((subcategory) => {
...
export { getProducts, getProductById, createProduct, updateProduct, deleteProduct, getCategories, getAvailableCategories, getTopSellingProducts, };
